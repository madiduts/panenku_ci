...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between h-32">
        ...
        <div>
            <h3 class="text-3xl font-bold text-gray-800"><?= number_format($stats['laporanPending']) ?></h3>
    ...
    <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between h-32">
        ...
        <div>
            <h3 class="text-3xl font-bold text-gray-800"><?= number_format($stats['disetujuiBulanIni']) ?></h3>
    ...
    <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between h-32">
        ...
        <div>
            <h3 class="text-3xl font-bold text-gray-800"><?= number_format($stats['ditolakBulanIni']) ?></h3>
            <p class="text-xs text-gray-400 mt-1">Bulan ini</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="overflow-x-auto">
        <?php if(empty($laporanMasuk)): ?>
            <div class="p-8 text-center text-gray-400">Tidak ada laporan yang menunggu validasi.</div>
        <?php else: ?>
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition group">
                    ...
                    <td class="px-6 py-4 text-gray-600"><i class="far fa-calendar text-gray-400 mr-1"></i> <?= date('d M Y', strtotime($item->tanggal)) ?></td>
                    <td class="px-6 py-4">
    ...
    <div class="overflow-x-auto">
        <?php if(empty($riwayatValidasi)): ?>
            <div class="p-8 text-center text-gray-400">Belum ada riwayat validasi.</div>
        <?php else: ?>
        <table class="w-full text-left text-sm">
            <thead class="bg-white text-gray-500 text-xs border-b border-gray-100">
                <tr>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-gray-600"><?= $item->komoditas ?></td>
                    <td class="px-6 py-4 text-gray-600"><?= date('d/m/Y', strtotime($item->tanggal)) ?></td>
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded text-xs font-bold flex items-center gap-1 w-fit <?= $item->status === 'Valid' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' ?>">
                            <i class="fas <?= $item->status === 'Valid' ? 'fa-check-circle' : 'fa-times-circle' ?>"></i> <?= $item->status ?>
                        </span>
                    </td>
...
<!-- SCRIPT KHUSUS HALAMAN VALIDASI -->
<script>
    function openModalReview(btn) {
        const data = btn.dataset;
        const container = document.getElementById('modal-container');
        
        // Perhatikan Form Action mengarah ke Dashboard_d/submit_validasi
        container.innerHTML = `
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
                <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
                    <form action="<?= base_url('dinas/dashboard_d/submit_validasi') ?>" method="POST">
                        <input type="hidden" name="panen_id" value="${data.id}">
                        
                        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                            <h3 class="font-bold text-gray-900 text-lg">Validasi Panen #${data.id}</h3>
                        ...
                        <div class="p-6 text-sm">
                            ...
                            <div>
                                ...
                                <textarea name="catatan" required class="w-full border border-gray-300 rounded-lg p-3 text-sm h-24 focus:ring-2 focus:ring-black focus:border-black outline-none resize-none" placeholder="Masukkan alasan persetujuan atau penolakan..."></textarea>
                            </div>
                        </div>

                        <div class="p-6 border-t border-gray-100 flex space-x-3 bg-gray-50">
                            <button type="submit" name="action" value="Valid" class="flex-1 bg-black text-white py-2.5 rounded-lg font-bold hover:bg-gray-800 shadow-md transition flex items-center justify-center gap-2">
                                <i class="far fa-check-circle"></i> Setujui
                            </button>
                            <button type="submit" name="action" value="Reject" class="flex-1 bg-red-600 text-white py-2.5 rounded-lg font-bold hover:bg-red-700 shadow-md transition flex items-center justify-center gap-2">
