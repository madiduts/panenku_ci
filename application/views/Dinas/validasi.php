<!-- 
    ...
    VERSION: Stable User Version + Lahan Feature Injected
-->
...
<div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
    ...
    <!-- Statistik Cepat (Updated dengan Lahan) -->
    <div class="flex space-x-6 text-sm">
        <!-- Statistik Lahan (BARU) -->
        <div class="text-right">
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Lahan Baru</p>
            <p class="text-xl font-bold text-blue-600"><?= isset($laporanLahan) ? count($laporanLahan) : 0 ?></p>
        </div>
        <div class="w-px bg-gray-200 h-10"></div>
        
        <div class="text-right">
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Hama</p>
            <p class="text-xl font-bold text-red-600"><?= isset($laporanHama) ? count($laporanHama) : 0 ?></p>
        </div>
        <div class="w-px bg-gray-200 h-10"></div>
        <div class="text-right">
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Panen</p>
            <p class="text-xl font-bold text-gray-800"><?= isset($laporanPanen) ? count($laporanPanen) : 0 ?></p>
        </div>
    </div>
</div>
...
<?php 
    ...
    $countPanen = isset($laporanPanen) ? count($laporanPanen) : 0;
    $countLahan = isset($laporanLahan) ? count($laporanLahan) : 0; // BARU
?>

<!-- NAVIGASI TAB (Updated dengan Lahan) -->
<div class="border-b border-gray-200 mb-6">
    <nav class="-mb-px flex space-x-6 overflow-x-auto">
        
        <!-- Tab Lahan (BARU) -->
        <a href="?tab=lahan" class="<?= $tab === 'lahan' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-blue-600' ?> whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm flex items-center transition">
            <i class="fas fa-map-marked-alt mr-2"></i> Validasi Lahan
            <?php if($countLahan > 0): ?><span class="ml-2 bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full text-[10px] animate-pulse">New</span><?php endif; ?>
        </a>

        <!-- Tab Panen -->
        <a href="?tab=panen" class="<?= $tab === 'panen' ? 'border-black text-black' : 'border-transparent text-gray-500 hover:text-gray-700' ?> whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm flex items-center transition">
            <i class="fas fa-wheat mr-2"></i> Laporan Panen
            <?php if($countPanen > 0): ?><span class="ml-2 bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-[10px]"><?= $countPanen ?></span><?php endif; ?>
        </a>

        <!-- Tab Hama -->
        <a href="?tab=hama" class="<?= $tab === 'hama' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-red-600' ?> whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm flex items-center transition">
            <i class="fas fa-bug mr-2"></i> Laporan Hama
            <?php if($countHama > 0): ?><span class="ml-2 bg-red-100 text-red-600 px-2 py-0.5 rounded-full text-[10px] animate-pulse"><?= $countHama ?> Baru</span><?php endif; ?>
        </a>
...
<div class="min-h-[400px] animate-fade-in-up">
    ...
    <?php if($tab === 'lahan'): ?>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-4 bg-blue-50/30 border-b border-gray-100">
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-blue-50/30 transition">
                                ...
                                <td class="px-6 py-4 font-mono font-bold"><?= htmlspecialchars($lahan->luas_lahan) ?> Ha</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <button onclick='openModalLahan(<?= json_encode($lahan) ?>)' class="bg-white border border-blue-200 text-blue-600 hover:bg-blue-600 hover:text-white px-4 py-1.5 rounded-lg text-xs font-bold transition shadow-sm">Verifikasi</button>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
    ...
    <!-- 2. TAB PANEN (KODE ASLI KAMU - TIDAK DIUBAH) -->
    ...
    <!-- 3. TAB HAMA (KODE ASLI KAMU - TIDAK DIUBAH) -->
    <?php elseif($tab === 'hama'): ?>
        <?php if(empty($laporanHama)): ?>
            <div class="bg-white rounded-xl border-l-4 border-green-500 p-10 text-center shadow-sm">
                ...
                <p class="text-gray-500 text-sm">Tidak ada laporan hama saat ini.</p>
            </div>
        <?php else: ?>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition">
                    ...
                    <div class="h-48 w-full bg-gray-100 relative cursor-pointer group" onclick='openModalHama(<?= json_encode($hama) ?>)'>
                        ...
                        <div class="absolute top-2 right-2">
                            <span class="bg-white/90 text-red-600 text-[10px] font-bold px-2 py-1 rounded uppercase shadow-sm border border-red-100">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-xs text-gray-500 mb-3">Oleh: <?= $hama->nama_petani ?></p>
                        <div class="flex items-center gap-2 text-xs text-gray-600 bg-gray-50 p-2 rounded mb-3">
                            <i class="fas fa-map-marker-alt text-red-400"></i> <?= $hama->lokasi_desa ?>
                        </div>
                        <button onclick='openModalHama(<?= json_encode($hama) ?>)' class="w-full border border-red-500 text-red-600 hover:bg-red-600 hover:text-white py-2 rounded-lg text-xs font-bold transition">
                            Validasi Laporan
                        </button>
                    </div>
                </div>
    ...
    <!-- 4. TAB RIWAYAT (KODE ASLI KAMU - TIDAK DIUBAH) -->
    <?php elseif($tab === 'riwayat'): ?>
        <div class="p-10 text-center text-gray-400 bg-white rounded-xl border border-gray-200">
            <i class="fas fa-history text-3xl mb-2"></i>
            <p>Riwayat validasi akan muncul di sini.</p>
        </div>
    <?php endif; ?>
...
<script>
                ...
                <form action="${BASE_URL}dinas/validasi/submit_validasi_lahan" method="POST" 
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar flex-1">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="catatan" required class="w-full border border-gray-300 rounded-xl p-3 text-sm h-20 focus:ring-2 focus:ring-blue-500 outline-none resize-none bg-gray-50" placeholder="Catatan..."></textarea>
                        </div>
                    </div>
                ...
                <form action="${BASE_URL}dinas/validasi/submit_validasi_hama" method="POST" 
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar flex-1">
                        ...
                        <div class="mb-4">
                            ...
                            <p class="text-xs text-gray-500">Dilaporkan oleh <span class="font-bold text-gray-800">${data.nama_petani}</span> di ${data.lokasi_desa}</p>
                        ...
                        <div>
                            ...
                            <textarea name="rekomendasi" required class="w-full border border-gray-300 rounded-xl p-3 text-sm h-28 focus:ring-2 focus:ring-red-500 outline-none resize-none shadow-sm transition bg-gray-50 focus:bg-white" placeholder="Contoh: Semprot pestisida X dosis 5ml/liter..."></textarea>
                        </div>
                    </div>
    ...
    // --- MODAL PANEN (SAMA SEPERTI ASLI) ---
    function openModalPanen(data) {
        const container = document.getElementById('modal-container');
        container.innerHTML = `
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
                <form action="${BASE_URL}dinas/validasi/submit_validasi_panen" method="POST" 
                      class="bg-white w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
