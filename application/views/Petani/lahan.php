    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in relative">
        ...
        <form action="<?= base_url('petani/lahan/simpan') ?>" method="POST">
            <div class="p-6 space-y-4">
                <div><label class="block text-xs font-bold text-gray-700 mb-1">Nama Lahan <span class="text-red-500">*</span></label><input type="text" name="nama_lahan" placeholder="Contoh: Lahan A-01" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-black focus:border-black outline-none"></div>
                <!-- ... Field lain disederhanakan agar tidak terlalu panjang, ikuti pola di atas ... -->
                 <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-xs font-bold text-gray-700 mb-1">Luas (ha)</label><input type="number" name="luas" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm"></div>
                    <div><label class="block text-xs font-bold text-gray-700 mb-1">Status</label><select name="status" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm"><option>Aktif</option><option>Istirahat</option></select></div>
                </div>
            </div>
            <div class="p-6 border-t border-gray-100 flex space-x-3">
                ...
                <button type="submit" class="flex-1 py-2.5 bg-black text-white rounded-lg text-sm font-bold hover:bg-gray-800 shadow-lg">Tambah Lahan</button>
            </div>
        </form>
    </div>
</div>